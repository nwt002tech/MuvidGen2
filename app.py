...
st.set_page_config(page_title="MuVidGen – Pixar 3D (FFmpeg-only)", layout="wide")
st.markdown("""
<style>
...
st.sidebar.write("🔧 Boot checkpoint: UI loaded (FFmpeg-only)")
...
def _st_energy_onsets(y: np.ndarray, sr: int, frame_len_s=0.046, hop_s=0.023):
    ...
    y = y / peak
    frame_len = int(frame_len_s * sr); hop = int(hop_s * sr)
    frame_len = max(frame_len, 1); hop = max(hop, 1)
...
def _peak_pick(onset: np.ndarray, hop_sec: float) -> List[float]:
    ...
    for i, val in enumerate(onset):
        if val > thr and (i - last) >= min_gap:
            neighborhood = onset[max(0, i-2):i+3]
...
st.title("🎬 MuVidGen — Pixar-Inspired 3D (FFmpeg-only)")
with st.sidebar:
    st.header("Settings")
    ...
    if st.button("Analyze Audio + Lyrics", type="secondary", use_container_width=True):
        ...
        if not audio_file or not lyrics.strip():
            ...
            with st.spinner("Analyzing lyrics…"):
                lines = chunk_lyrics_to_lines(lyrics); kw = extract_keywords(lyrics, top_k=14)
                tags = []
                for ln in lines:
...
if render_btn:
    ...
    if not st.session_state.get("analysis") or not st.session_state.get("story"):
        ...
        for i, line in enumerate(lines):
            ...
            img = None
            if not force_fallback and (st.secrets.get("HF_TOKEN","") or "").strip():
                img = hf_txt2img(prompt, st.secrets["HF_TOKEN"], model_id=model_id.strip() or "stabilityai/sdxl-turbo")
            else:
                if force_fallback: log("Force fallback is ON")
