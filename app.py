import os, io, re, uuid, time, tempfile, logging, sys
...
st.set_page_config(page_title="MuVidGen – Pixar 3D (Py3.13-safe)", layout="wide")
st.markdown("""
<style>
...
logger = logging.getLogger("muvidgen")
logger.setLevel(logging.INFO)
handler = logging.StreamHandler(sys.stdout)
handler.setLevel(logging.INFO)
logger.handlers = [handler]
def log(msg: str):
    logger.info(msg); st.sidebar.write(f"🧭 {msg}")
st.sidebar.write("🔧 Boot checkpoint: UI loaded (Py3.13-safe)")
...
def _frame_signal(y: np.ndarray, sr: int, frame_hop: int, frame_len: int):
    n_frames = 1 + (len(y) - frame_len) // frame_hop if len(y) >= frame_len else 1
    for i in range(n_frames):
        start = i * frame_hop
        end = start + frame_len
        if end > len(y):
            chunk = np.zeros(frame_len, dtype=y.dtype)
            chunk[:len(y)-start] = y[start:]
        else:
            chunk = y[start:end]
        yield chunk

def _envelope(y: np.ndarray, sr: int):
    ...
    hop = int(0.023 * sr)
    energies = []
    for frame in _frame_signal(y, sr, hop, frame_len):
        energies.append(np.sqrt(np.mean(frame**2)))
    env = np.array(energies, dtype=np.float32)
    win = 5
    if len(env) > win:
...
def _peak_pick(onset: np.ndarray, hop_sec: float) -> List[float]:
    thr = np.percentile(onset, 80) * 0.7
    peaks = []
    min_gap = int(0.2 / hop_sec)
    last = -min_gap
    for i, val in enumerate(onset):
        if val > thr and (i - last) >= min_gap:
            left = onset[max(0, i-2):i+3]
            if len(left) and val >= left.max():
                peaks.append(i); last = i
    return [p * hop_sec for p in peaks]
...
def analyze_audio(file_bytes: bytes, sr_target: int = 44100) -> AudioAnalysis:
    log("Reading audio via soundfile")
    with tempfile.NamedTemporaryFile(suffix=".tmp", delete=False) as tmp:
        tmp.write(file_bytes); tmp.flush()
        y, sr = sf.read(tmp.name, dtype="float32", always_2d=False)
    if hasattr(y, "ndim") and y.ndim > 1: y = y.mean(axis=1)
    if sr != sr_target:
        log(f"Resampling {sr}→{sr_target}")
        ...
        sr = sr_target
    duration = len(y) / sr
    onset, hop_sec = _envelope(y, sr)
    peak_times = _peak_pick(onset, hop_sec)
    if len(peak_times) < 4 and duration > 4:
        n = len(onset)
        if n > 32:
            f = np.fft.rfft(onset - onset.mean())
            ac = np.fft.irfft(f * np.conj(f))
            ac[:int(0.2/hop_sec)] = 0
            lag = np.argmax(ac[:int(2.0/hop_sec)])
            est_int = max(0.5, min(1.0, lag * hop_sec))
            beat_times = list(np.arange(0, duration, est_int))
        else:
            beat_times = peak_times
    else:
        beat_times = peak_times
    tempo_bpm = _estimate_bpm(beat_times)
    log(f"Audio analyzed: duration={duration:.2f}s bpm≈{tempo_bpm:.1f} beats={len(beat_times)}")
    return AudioAnalysis(duration=float(duration), tempo_bpm=float(tempo_bpm), beat_times=[float(t) for t in beat_times])
...
def build_prompt(line, tags, global_kw, style: StyleSettings) -> str:
    kw_part = ", ".join(sorted(list(set((tags or []) + global_kw[:5]))))
    bible = (style.character_bible or "").strip()
    main = (
        f"{style.visual_style}, {style.characters}. "
...
def hf_txt2img(prompt: str, hf_token: str, model_id: str = "stabilityai/sdxl-turbo", width=768, height=512, steps=6, guidance=3.0):
    ...
    try:
        ...
        log(f"HF call starting → model={model_id}")
        r = requests.post(url, headers=headers, json=payload, timeout=120)
        if r.status_code == 200 and r.headers.get("content-type","").startswith("image/"):
            log("HF call OK (image bytes received)")
            return Image.open(io.BytesIO(r.content)).convert("RGB")
        if r.status_code == 503:
            log("HF 503: retrying once after short wait…"); import time; time.sleep(3)
            ...
            if r2.status_code == 200 and r2.headers.get("content-type","").startswith("image/"):
                log("HF retry OK (image bytes)")
                return Image.open(io.BytesIO(r2.content)).convert("RGB")
        log(f"HF call failed: status={r.status_code} content-type={r.headers.get('content-type','')}")
        return None
    except Exception as e:
        log(f"HF exception: {e}"); return None

def fallback_card(text: str, size=(768, 512)):
    from PIL import Image, ImageDraw, ImageFont
    log("Using fallback card for a scene")
    w, h = size
    ...
    try: font = ImageFont.truetype("DejaVuSans.ttf", 28); font2 = ImageFont.truetype("DejaVuSans.ttf", 24)
    except: font = ImageFont.load_default(); font2 = ImageFont.load_default()
    title = "Pixar-inspired Scene"
    bbox = draw.textbbox((0,0), title, font=font); tw, th = bbox[2]-bbox[0], bbox[3]-bbox[1]
...
def subtitle_clip(text: str, width: int, height: int, duration: float):
    ...
    for w_ in words:
        cur.append(w_); 
        if len(" ".join(cur)) > max_chars: lines.append(" ".join(cur)); cur = []
    if cur: lines.append(" ".join(cur))
...
st.title("🎬 MuVidGen — Pixar-Inspired 3D (Py3.13-safe)")
with st.sidebar:
    st.header("Settings")
    ...
    DEFAULT_MODEL_HINT = "family-safe, cheerful, vibrant, consistent stylization"
    DEFAULT_NEGATIVE = "no text, no watermark, no logo, no signature, no disfigured hands, no scary, no photoreal humans, no copyrighted characters"
    visual_style = st.text_area("Base style", value=DEFAULT_VISUAL_STYLE, height=90)
    characters = st.text_input("Characters", value=DEFAULT_CHARACTERS)
    palette = st.text_input("Color Palette", value=DEFAULT_PALETTE)
    camera = st.text_input("Camera Direction", value=DEFAULT_CAMERA)
    model_hint = st.text_input("Model Hint", value=DEFAULT_MODEL_HINT)
    ...
    negative_hint = st.text_input("Avoid / negatives", value=DEFAULT_NEGATIVE)
...
def build_style(s): 
    return StyleSettings(s["visual_style"], s["characters"], s["palette"], s["camera"], s["model_hint"], s.get("character_bible",""), s.get("negative_hint",""))

def fallback_card_public(text: str): return fallback_card(text)

def subtitle_clip_public(*args, **kwargs): 
    return subtitle_clip(*args, **kwargs)

def assemble_video_public(*args, **kwargs):
    return assemble_video(*args, **kwargs)

def build_prompt_public(*args, **kwargs):
...
if render_btn:
    ...
    if not st.session_state.get("analysis") or not st.session_state.get("story"):
        ...
        style = build_style(st.session_state.style)
        ...
        gen_progress = st.progress(0); images = []; total = max(1, len(lines))
        use_hf = (st.secrets.get("HF_TOKEN","") or "").strip() != ""
        for i, line in enumerate(lines):
            prompt = build_prompt_public(line, tags[i] if i < len(tags) else [], kws, style)
            img = None
            if use_hf: img = hf_txt2img(prompt, st.secrets["HF_TOKEN"], model_id=model_id.strip() or "stabilityai/sdxl-turbo")
            if img is None: img = fallback_card_public(line)
        ...
        with st.spinner("Assembling video (beat-aware cuts + subtitles)…"):
            out_path, vid_dur = assemble_video_public(images, lines, audio_path, beat_times, total_duration=duration)
