...
# ---------- Page & Mobile tweaks ----------
st.set_page_config(page_title="MuVidGen – Pixar 3D (Full, Optimized)", layout="wide")
st.markdown("""
<style>
...
# ---------- UI ----------
st.title("🎬 MuVidGen — Pixar-Inspired 3D (Full, Optimized)")
...
with st.sidebar:
    ...
    model_id = st.text_input(
        ...
        help="Any text-to-image model with Inference API enabled."
    )
    ...
    character_bible = st.text_area("Character Bible (consistency)", value="Green letter F (forward-hook top curve), kid-safe smile, consistent eyes/mouth; rounded limbs; classroom setting.", height=90)
...
with colB:
    render_btn = st.button("Generate Full Music Video", type="primary", use_container_width=True, disabled=not st.session_state.get("analysis"))
...
if render_btn:
    if not st.session_state.get("analysis") or not st.session_state.get("story"):
        ...
        style = StyleSettings(
            visual_style=st.session_state.style["visual_style"],
            characters=st.session_state.style["characters"],
            palette=st.session_state.style["palette"],
            camera=st.session_state.style["camera"],
            model_hint=st.session_state.style["model_hint"],
            character_bible=st.session_state.style.get("character_bible",""),
            negative_hint=st.session_state.style.get("negative_hint","")
        )
